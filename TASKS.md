...
- [ ] **T37 — Merchant portal** — profile, styles, reward rules, visits, staff, billing.
  ...
  ⚠️ **Read before starting — this one is NOT unblocked the way T36 was.** T36 needed two endpoints that did not exist and could be built in an afternoon (T42/T45). T37 has a bigger hole, checked in the files 2026-08-24, not assumed:

  | Portal area | Backend today | Work needed |
  |---|---|---|
  | Profile | `GET /merchants/me` (read-only) | ⚠️ **No `PATCH`/`PUT` exists** — a salon cannot edit its own name or details. See the scoping note below. |
  | Styles | `GET /styles`, `POST /styles`, `PATCH /styles/:id`, `PATCH /:id/{activate,deactivate}` | ✅ nothing |
  | **Reward rules** | 🔴 **Nothing at all** | **A whole controller AND the service CRUD.** `src/modules/reward-rules/` contains a module and a service, and the service has exactly one method — `evaluate()`, which `POST /visits` calls to decide what just unlocked. There is **no** create / list / update / deactivate, and `reward-rules.module.ts` declares no `controllers` array. This is the same fact that made [F30]'s paywall registration meaningless (`reward-rules/(.*)` matched nothing because nothing was there to match). |
  | Visits | `GET /visits`, `POST /visits` | ✅ nothing |
  | Staff | 9 routes (T24) | ✅ nothing |
  | Billing | checkout / cancel / resume (T17, T24) | ✅ nothing |

  **So T37 is really two tasks in a trench coat:** build the reward-rules HTTP layer, then wire the portal. Expect that to be closer in size to T36 than to T38.

  **Scoping call to make first, not mid-task — does "profile" mean *edit*?** The prototype's own business portal has four tabs (log visit, styles, rules, ledger) and **no profile editor**, so `GET /merchants/me` may already satisfy the word. If editing is wanted, it is a new endpoint plus a decision about which fields a merchant may change themselves (`businessName` yes; `email` is the login identity and `status` is the admin's to set — neither should be self-serve). Decide before building.

  **Two guard rails already paid for — reuse them, do not re-derive them.** `RequireMerchantGuard` accepts owner *and* staff for day-to-day work; `RequireMerchantOwnerGuard` refuses staff on the things only an owner should do (T24 narrowed billing onto it — a receptionist could previously have cancelled the salon's subscription). A new reward-rules controller must pick one per route deliberately, and must not repeat the `req.merchantId!` pattern behind [F29].
- [ ] **T38 — Admin panel** — approval queue, MRR/churn metrics. ✅ **Backend is already complete** — T22 built admin login plus `GET /admin/merchants/pending`, `PATCH /admin/merchants/:id/{approve,suspend}`, and `GET /admin/metrics/{mrr,churn,platform}`, all behind `RequireAdminGuard`. This is frontend-only, and `glow-plus-web/admin.html` → `/admin/panel` (T22) already calls every one of them, so it is largely a matter of moving that into the SPA's `Admin.jsx` view.
