...
| F63 | ⚠️ **FIXED 2026-08-26 — was open since session 28.** **Appointment and slot times rendered in the BROWSER's timezone, not the salon's.** [F57] had already fixed the write half: `availability.service.ts` resolves `"09:00"` against the salon's timezone and returns real UTC instants. The read half then undid it — the client called `toLocaleTimeString(undefined, …)`, which formats in **the viewer's** zone. So a Toronto salon offering **9:00 AM** displayed as **6:00 PM** to a customer in Karachi, and the two halves of one feature disagreed about what "9am" meant. **Worse, and not noticed until the fix was written:** `formatDay` had the same flaw, so a visit at **11:30 PM Toronto on Aug 31** was filed under **Sep 1** — the wrong DAY, not merely the wrong time | ✅ **FIXED** — new `SALON_TIMEZONE` in `lib/config.js`, defaulting to `America/Toronto` to **match the backend's own default** (the country the platform sells in — prices are CAD), overridable via `VITE_SALON_TIMEZONE`. All three formatters now pass `timeZone`, and the two that show a clock also pass **`timeZoneName: 'short'`** — a bare *"2:30 PM"* is read as local time by everyone who sees it, so the zone has to be said out loud (*"9:00 AM EDT"*). ⚠️ **This constant must track the backend's `SALON_TIMEZONE`**; they share a default rather than one falling back to UTC, precisely so drift cannot silently reintroduce this. The platform is single-timezone by design for now — when `Merchant.timezone` lands, this becomes the fallback rather than the answer | `lib/config.js`, `lib/helpers.js` |
| F76 | 🔴 **THE MOST SERIOUS OF THE RUN — the salon had NO screen for its own appointments.** Found by the user asking a plain question: *"where should I see the booking in salon side?"* There was nowhere. `GET /bookings` (merchant-guarded, paginated, date-filtered) shipped in **T18**, and `PATCH /:id/confirm` and `PATCH /:id/cancel` beside it — and **no client ever called any of the three.** The portal had **nine tabs** and not one was Appointments; it never fetched bookings at all. **A customer books 9am Monday and the salon never finds out.** Third instance of the shape behind [F60] (the salon could not see redemptions) and [F75] (the customer could not see their own), and the worst of the three: a redemption records something that **already happened**, but a booking is a **commitment about the future**, and a salon that cannot see it does not keep it. It also lands squarely on the client's own brief — *"Booking flow — built and tested by me against a mock backend, but never run against your real Postgres"*: it runs against Postgres now, into a table nobody could read | ✅ **FIXED** — an **Appointments** tab in the portal (`BookingsPanel`): time, client **name + email**, service, status, with **Confirm** and **Cancel** wired to the endpoints that had never been called. Defaults to **today onward** (local midnight, not UTC) because an appointment book is forward-looking. Shows the **email, not the phone** — the phone is AES-GCM encrypted at rest (T31b) and deliberately never returned. Verified live: the customer's booking appeared, and confirming it through the new button moved it to **CONFIRMED** in Supabase — the first call `PATCH /:id/confirm` has ever received | `views/BusinessPortal.jsx`, `lib/api.js` |
| F75 | ⚠️ **[F60] WITH THE ROLES REVERSED — found by the user during the P-series run.** Redeeming raised a toast reading ***"Show this to the salon"*** — and then **the toast vanished and there was nothing to show.** The SPA consumer dashboard had **no redemption history at all**, so the one instruction the product gave the customer was not actionable. `GET /redemptions/me` **and** `listMyRedemptions()` both already existed and worked; the only client calling them was **`/consumer/rewards`**, a standalone page built before T35's auth UI and **never linked from the SPA**. So the data was reachable and the customer was not. F60 read: *"a customer redeemed 20% off, saw a toast that vanished, and the salon had nowhere to check."* The salon got its Redemptions tab in T63's session; **the customer never got theirs** | ✅ **FIXED, both halves.** A **Redeemed** tab in the SPA dashboard (`RedeemedPanel`, modelled on `VisitsPanel`, gated on `currentConsumer` per [F69], scrolling inside its own box for 390px), and the toast now says *"It is saved under Redeemed"* — a promise the product can keep. ⚠️ **The API needed a change too:** `/redemptions/me` returned the reward's name but **not the salon's** — and this is a **cross-salon** history, so *"20% off"* with no salon beside it is not actionable at a counter. `history()` now includes `businessName`, flattened onto the row. Verified live: `2026-08-26 \| Bloom Hair Studio \| 20% off after 3 visits`. 474 tests still green | `views/ConsumerDashboard.jsx`, `redemptions.service.ts` |
| F74 | ⚠️ **PARTIALLY FIXED — the rest is a PRICING decision, not a code one. Found by the user during the P-series production run.** **Three problems, one root question: what is the actual offer?** **(a) The site contradicted ITSELF on the same page** — the promo bar and Founding section both said *"every new salon gets their **first month** free"* while the pricing card said *"**7-day** free trial"*. **(b) The code matches neither for most salons:** `trialDays = STANDARD_TRIAL_DAYS(7) + (foundingMember ? 30 : 0)`, so the **first 50 get 37 days** (promise honoured) and **the 51st onward get 7** — the moment the 51st salon signs up the site advertises a free month and the billing code grants a week, which is a consumer-protection exposure rather than a copy nit. **(c) A salon that NEVER subscribes is never blocked at all:** the free period exists only as Stripe's `trial_period_days`, so with no checkout there is no subscription, no trial, no trial end and no webhook — and `RequireActiveSubscriptionGuard` **never consults the `Subscription` table**, refusing only SUSPENDED/CANCELLED/PAST_DUE. An `ACTIVE` merchant with no subscription passes straight through. **Confirmed live:** the P-series test salon is ACTIVE, publicly listed, full portal access, `subscription: NONE`. No cron job downgrades it either — `trialEndingReminder` only looks at subscriptions already `TRIALING`. **This is the exact twin of [F50]:** F50 is *paying bypasses approval*, this is *approval bypasses paying*. The two gates were built independently and neither requires the other | ✅ **(a) FIXED IN ALL 8 LANGUAGES.** `en` states the 7-day trial with the founding month **on top of** it. For the other 7 the false clause was **DELETED rather than translated** — a safe fix, because it turned out not to need one: every language already states *"7-day free trial"* **correctly** on the pricing card, so the accurate figure was never missing, only contradicted. `promo_bar` was truncated at its em-dash (all 8 share that structure) and `founding_sub`'s **first sentence** was dropped (all 8 are two sentences, and only the first was false). What remains in each is the badge claim, which is **true of exactly the salons it describes**. ⚠️ **Deletion, not invention** — machine-translating a pricing promise is where guessing is unacceptable, and this needed neither. ⬜ The 7 languages now say *less* than English; restoring the fuller founding offer in each needs the client's translator, but nothing false remains.
      ⚠️ **A FOURTH instance found while fixing this:** `Admin.jsx` appended **`meta_first_month_free` unconditionally** to every pending salon in the review list — the Founding badge two lines above it *was* gated on `m.foundingMember`, the suffix was not. So an admin reviewing salon **#51** was told they get a free month when they get seven days. ✅ **Fixed**, gated the same way as the badge beside it. The portal's own pending banner was already correct (`foundingMember ? founding : standard`). ⬜ **(b) and (c) are the CLIENT's call** — either fix the copy to 7 days or raise `STANDARD_TRIAL_DAYS` to 30 and give away three extra weeks per salon; and decide whether approval should grant a grace period, be merged with subscribing, or leave the portal open while the salon stays hidden. Deliberately not changed mid-handover: blocking salons is revenue-affecting and could lock out legitimate users | `i18n/translations.js`, `billing.service.ts`, `require-active-subscription.guard.ts` |
| F73 | ⚠️ **A test that passed for the wrong reason.** While writing T65, the helper picking "next Monday" used `(dow - today) % 7`. **JavaScript's `%` keeps the sign of the dividend**, so on a Wednesday `(1 - 3) % 7` is `-2`, not `5` — it resolved to Monday of *this* week, already gone. The booking test failed honestly on that. **The closed-Sunday test beside it PASSED**, because it too asked about a date in the past and a past date also returns `[]` — reporting green while proving nothing about the closed-day rule it exists to check | ✅ **FIXED** — `((dow - today + 7) % 7) \|\| 7`, and the closed-day test now additionally asserts the date it picked really is a **future** Sunday, so it can never again pass because the day had simply gone by | `src/e2e.spec.ts` |
| F72 | ⚠️ **OPEN — a design property, not a bug, and the client's call.** **`POST /merchants/signup` calls `stripe.customers.create()` BEFORE writing the salon row**, so **a Stripe outage blocks every salon signup**. The verification email sitting immediately beside that call *is* wrapped in `try/catch` precisely so a Resend outage cannot do this — the Stripe call has no such guard, so the asymmetry looks accidental. Surfaced by T65: the endpoint returns **500** in CI, where no Stripe key exists. Deferring customer creation to checkout would remove the dependency entirely, at the cost of a little complexity there | ⬜ **Client decision.** Not changed mid-handover: it is a deliberate-looking ordering, and altering signup's failure modes days before launch is riskier than the outage it guards against | `modules/merchants/onboarding.service.ts` |
| F71 | ⚠️ **OPEN — needs a CLIENT decision, and it is the twin of [F50].** **A customer keeps full sight of points and rewards at a salon that has stopped paying.** The moment a salon goes PAST_DUE or CANCELLED it vanishes from the public directory and its menu 404s — but **`/me/rewards` does not filter by merchant status**, so the customer still sees the salon, still sees *"Ready — 20% off"*, and still sees a **Redeem button**, for a salon that has disappeared from the app and whose staff can no longer log visits. Not a crash, and arguably defensible — they *did* earn it — but right now the product silently pretends nothing happened. Three questions the client must answer: should points survive a salon leaving? should the reward stay redeemable? should it instead read *"this salon is no longer on Glow+"*? | ⬜ **Client decision**, alongside [F50] | `modules/me/me.service.ts` |
| F69 | ⚠️ **FIXED 2026-08-26 (found by the user on the live site).** **The landing page greeted every first-time visitor with a toast reading `Missing bearer token`.** `App.jsx` keeps **every view mounted from first paint** — `.view.active` toggles *visibility, not existence* — so `ConsumerDashboard` runs its loaders while an anonymous visitor is looking at the marketing page. Three of them (`bookings`, `visits`, `rewards`) are protected routes, so each returned **401**, and `useApiError` toasts the API's own words **unconditionally**. The sign-out beside it was already guarded on `currentConsumer` *for exactly this reason*, and both sibling views — `BusinessPortal`'s `usePortalData` and `Admin`'s `useAdminData` — gate the **fetch** as well, each with a comment explaining that the gate is load-bearing. `ConsumerDashboard`'s `useApiData` was **the only one of the three that guarded the consequence but not the request.** ⚠️ **It could not have been caught locally:** a developer's browser almost always holds a consumer token from an earlier sign-in, so the loaders succeed. It needs a genuinely fresh browser, which is what a production URL supplies — the first thing a real visitor gets, and the one visitor no local test ever simulates | ✅ **FIXED — in TWO places; the first fix was incomplete and the user caught that too.** **(a)** `useApiData` in `ConsumerDashboard` now does `if (!currentConsumer) return null;`, mirroring `usePortalData`. **(b)** `HoursPanel` in `BusinessPortal` called `getMyBusinessHours()` from a bare `useEffect`. It opts out of `usePortalData` for a **good** reason — the form is editable, so re-reading on every `dataVersion` bump would discard unsaved typing — but opting out of the hook also opted out of its **session gate**, which was the half that mattered. Now gated on `currentMerchant`. ⚠️ **The lesson: the gate belongs to the FETCH, not to the hook.** Any bare `useEffect` that calls a protected route from an always-mounted view needs it too. Audited every `useEffect` in all three protected views afterwards — the only other direct call is `listPublicStyles()`, which is `auth: false`. Rebuilt and redeployed | `views/ConsumerDashboard.jsx`, `views/BusinessPortal.jsx` |
...
- [x] **T62 — Backups, monitoring, logging.** ✅ **DONE 2026-08-26.** Three separate concerns; two were already in place and the third was a real, live risk.

      **⚠️ BACKUPS — this was the genuine gap. Supabase's free plan takes NO backups at all:** no daily snapshot, no point-in-time recovery. A dropped table or a bad migration is simply gone. **The correct fix is the Pro plan (~$25/mo)**, which adds daily backups and 7-day PITR — a **client decision about money**, not something code can solve, and it should be made before real customer data exists.
      **Built as the stopgap:** `npm run backup` (`scripts/backup.ts`) — a complete logical snapshot of all 15 tables to timestamped JSON, runnable from any machine. Tables are dumped in **topological order** (parents before children) and that order is recorded in the file, so a restore never points a foreign key at a row that does not exist yet.
      ⚠️ **Be honest about its limits, which the script's header states:** it dumps **data, not schema** (restore assumes `prisma migrate deploy` has run), it is a point-in-time copy taken *when a human remembers*, and it does not cover Supabase-side auth/storage objects. **A safety net, not a substitute for real backups.**
      ⚠️ **The output is SENSITIVE** — bcrypt hashes and AES-GCM phone ciphertext. `backups/` is gitignored (verified: the dump does not appear in `git status`), and the connection string in the file's metadata is **masked**. The ciphertext is useless without `ENCRYPTION_KEY`, which is exactly why that key must never be stored beside a dump.
      **Verified against production:** 2 rows across 15 tables, 1.6 KB — matching the known post-T63 state (the client's `Admin` and its refresh token).

      **MONITORING — the endpoint already exists, nothing is watching it.** `GET /health/ready` returns **200** with `database.latencyMs`, or **503** with the Prisma error code when Postgres is unreachable — the right shape for an uptime probe, and `VERSION_NEUTRAL` so it survives a future `/v2`. ⬜ **Client action:** point an external monitor (UptimeRobot, Better Stack — both have free tiers) at `https://glow-plus-api.vercel.app/health/ready`. Deliberately **not** built in-house: a monitor that runs inside the thing it monitors cannot report that the thing is down.
      ⚠️ **Also flag to the client:** the free Supabase project **autosuspends after ~1 week idle**, so a quiet launch week can look exactly like an outage. An uptime monitor's traffic incidentally prevents this, which is a second reason to set one up.

      **LOGGING — already adequate, verified rather than assumed.** `AllExceptionsFilter` logs **5xx as `error` with the stack** and **4xx as `warn`** with method, URL and status; Vercel captures stdout/stderr per invocation and exposes it via `vercel logs`. The cron dispatcher additionally returns a per-job summary in its response body, which is what Vercel's cron log displays — so a failing nightly job is visible without digging.
...
- [x] **T65 — Integration tests.** ✅ **DONE 2026-08-26.** `src/e2e.spec.ts` — **15 cases** that boot the **real** Nest app over a **real** Postgres and drive it by **real HTTP**, run in CI against a `postgres:16` service container with the real migrations applied first.
      **Why they are not more unit tests:** the other 29 suites mock Prisma and assert one unit. These exercise the seams a unit test cannot see — middleware ordering, the guard chain, URI versioning, the error envelope, bcrypt, AES-GCM at rest, foreign keys. **Every bug T63 found on production lived in exactly that gap:** each unit passed and the assembled system still misbehaved.
      **Coverage across the six areas named:** auth (signup, duplicate 409, login, wrong password, and **bcrypt + ciphertext asserted with RAW SQL** so no application-layer decryption can hide what is on disk) · authz (no token, junk token, a **consumer token refused on a merchant route**) · merchant lifecycle (PENDING hidden → ACTIVE listed → SUSPENDED hidden **and its writes 403**) · rewards (30 points over 3 visits → unlock → redeem → second attempt refused) · bookings (**[F64]**'s write-path hours enforcement, plus a real slot and its double-book) · the webhook signature · and the error envelope with its security headers.
      ⚠️ **`E2E_REQUIRE=1` in CI makes an unreachable database FATAL.** The first run passed 15 tests **having never connected to anything** — a suite that reports green for precisely the integration it failed to test is worse than no suite. Locally, with Docker stopped, it warns and no-ops so `npm test` stays runnable.
      ⚠️ **The salon row is created directly, not through `POST /merchants/signup`** — see [F72]: that endpoint calls Stripe before writing, so requiring it would mean the suite could not run from a fork or offline.
      **Two failures found while writing them, both worth keeping:** the negative-modulo bug below ([F73]), and the Stripe coupling ([F72]).
