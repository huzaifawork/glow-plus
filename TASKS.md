...
- [ ] **T44 — `GET /styles/public/:merchantId`** — current `/styles` is merchant-scoped _and_ behind `RequireActiveSubscription`, so it can't serve consumers.
      **Still open after T43, and deliberately.** T43 moved the *directory* onto the path the RN app calls and took `styleCount`/`styleTypes` with it, which removed the salon grid's need for this route — but not the consumer's: the booking flow and the dashboard's style picker need each style's `id`, `name`, `pointsPerVisit` and `durationMinutes`, which a directory row does not carry. The T18 stopgap at `GET /styles/public/:merchantId` is untouched and still serving them. What T44 owes it is the same treatment T43 just had — validated params and a decision about whether it, too, should be reachable under a path the RN app already names (`client.js:157` calls exactly this one, so unlike T43 the path is already right).
