...
| Tests | ✅ Jest configured, **189 passing** (`npm test`) — 15 suites: jwt.util (**23, rewritten T30**), health controller, exception filter (**+6 body-parser, T31**), billing readPeriod, require-merchant / require-consumer / require-admin / require-merchant-owner / require-active-subscription guards, trialEndingReminder job, expirePoints job, throttling, env.validation, security headers/CORS, **input-validation (T31, 22)**. ⚠️ `jest.setup.ts` supplies `JWT_SECRET` — jwt.util has no fallback |
...
➡️ **NEXT: T31b (PII at rest — phone numbers are plaintext; needs a client decision) and T32 (M-Pesa — needs a client decision), or skip both and start Phase 5 (T33+, the website build).** Check `TASKS.md` for the next unticked item. **T31c** (Nest 11 / Vite 8 majors) is open and should land before the client runs their own `npm audit`.
