...
| Tests | ✅ Jest configured, **146 passing** (`npm test`) — 10 suites: jwt.util, health controller, exception filter, billing readPeriod, require-merchant guard, require-consumer guard, require-admin guard, **require-merchant-owner guard (T24)**, trialEndingReminder job, **expirePoints job (T25)**, **throttling (T26, 18)**, **env.validation (T27, 19)**, **security headers/CORS (T28, 26)**, **require-active-subscription guard (T29, 13)**. ⚠️ `jest.setup.ts` now supplies `JWT_SECRET` — jwt.util no longer has a fallback |
...
➡️ **NEXT: T30 (consider `jsonwebtoken` over the hand-rolled HS256 — [F12]), then T31 (security review pass).** Check `TASKS.md` for the next unticked item. Note [F28] (signup's check-then-create race) is explicitly parked for **T31**.
